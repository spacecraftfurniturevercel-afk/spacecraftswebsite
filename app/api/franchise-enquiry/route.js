...
async function sendEmail({ to, subject, html }) {
  if (!EMAIL_APP_PASSWORD) {
    console.warn('[franchise-enquiry] EMAIL_APP_PASSWORD not set — skipping email to:', to)
    return { ok: false }
  }
  try {
    ...
    console.log('[franchise-enquiry] Email sent:', info.messageId, 'to:', to)
    return { ok: true }
  } catch (err) {
    console.error('[franchise-enquiry] Email error:', err.message)
    return { ok: false }
  }
}
...
export async function POST(request) {
  try {
    ...
    const { name, email, phone, company_name, gst_number, city, state, investment_range, space_available, message } = body
...
<tr><td style="background:#1a1a1a;padding:24px 32px;">
  ...
  <p style="margin:4px 0 0;font-size:13px;color:rgba(255,255,255,0.6);">New Franchise Application</p>
</td></tr>
...
<tr><td style="background:#f9f9fb;padding:20px 32px;border-top:1px solid #eee;">
  <p style="margin:0;font-size:12px;color:#999;text-align:center;">Submitted via <a href="${SITE_URL}/franchise" style="color:#2563eb;text-decoration:none;">${SITE_URL}/franchise</a></p>
</td></tr>
</table>
</td></tr>
</table>
    ...
    await sendEmail({
      to: ADMIN_EMAIL,
      subject: `🏪 Franchise Application from ${name}${company_name ? ` (${company_name})` : ''} — ${city}, ${state}`,
      html,
    })
