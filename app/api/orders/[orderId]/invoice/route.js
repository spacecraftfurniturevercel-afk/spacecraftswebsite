 ...
 * Generate and return a professional A4 HTML invoice
 */
export async function GET(request, { params }) {
  try {
    ...
    // Fetch delivery address via service role (bypasses RLS)
    let address = null
    if (order.address_id) {
    ...
    const subtotal   = Number(order.subtotal)        || items.reduce((s, i) => s + i.unit_price * i.quantity, 0)
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .inv-brand-tag { font-size: 11px; color: #aaa; letter-spacing: 0.5px; }
    ...
    /* â”€â”€ Shipment + Payment strip â”€â”€ */
    .info-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 0;
...
<body>
  ...
  <div class="no-print" style="text-align:center;padding:16px 0 0;font-family:Inter,sans-serif;">
    <button onclick="window.print()" style="background:#1a1a1a;color:#fff;border:none;padding:10px 28px;border-radius:6px;font-size:14px;font-weight:600;cursor:pointer;letter-spacing:0.3px;">
      â¬‡ Download / Print Invoice
    </button>
  </div>
