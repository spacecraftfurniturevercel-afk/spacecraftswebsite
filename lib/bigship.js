...
export async function createBigShipOrder({
  ...
  const totalWeight = items.reduce(
    (w, item) => w + (item.weight || 5) * item.quantity,
    0
  )

  // Furniture typically: estimate box size from weight
  const estimatedLength = Math.max(30, Math.min(120, totalWeight * 5))
  const estimatedWidth = Math.max(20, Math.min(80, totalWeight * 3))
  const estimatedHeight = Math.max(20, Math.min(100, totalWeight * 4))
