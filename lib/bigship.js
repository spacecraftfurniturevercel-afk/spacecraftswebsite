...
export async function createBigShipOrder({
  ...
  // BigShip requires strict consistency across all 3 levels:
  //   shipment_invoice_amount == each_box_invoice_amount * box_count
  //   each_box_invoice_amount == sum(each_product_invoice_amount)
  //   total_collectable_amount == each_box_collectable_amount * box_count  (COD only)
  //   total_collectable_amount <= shipment_invoice_amount
  //
  // Strategy:
  //   Prepaid → box/shipment invoice = product subtotal only
  //   COD     → box/shipment invoice = totalAmount (subtotal + GST + delivery)
  //             so total_collectable_amount (= totalAmount) == shipment_invoice_amount ✓
  const productSubtotal = items.reduce((s, item) => s + item.unit_price * item.quantity, 0)
  const boxInvoiceTotal = isCod ? totalAmount : productSubtotal

  // Distribute invoice proportionally across products; assign rounding remainder to last item
  const rawProducts = items.map((item) => {
    const baseAmount = item.unit_price * item.quantity
    const invoiceAmount = (isCod && productSubtotal > 0)
      ? Math.round(baseAmount / productSubtotal * boxInvoiceTotal)
      : baseAmount
    return {
      category: 'Others',
      subCategory: '',
      ...
      invoiceAmount,
      collectableAmount: isCod ? invoiceAmount : 0,
      hsn: item.hsn || '',
    }
  })

  // Correct any rounding drift so sum(product.invoiceAmount) === boxInvoiceTotal exactly
  const products = rawProducts.map((p, idx) => {
    if (idx < rawProducts.length - 1) return p
    const sumBefore = rawProducts.slice(0, -1).reduce((s, x) => s + x.invoiceAmount, 0)
    const lastAmount = Math.max(1, boxInvoiceTotal - sumBefore)
    return { ...p, invoiceAmount: lastAmount, collectableAmount: isCod ? lastAmount : 0 }
  })
  ...
  const boxDetails = [
    {
      ...
      width: maxWidth,
      height: maxHeight,
      invoiceAmount: boxInvoiceTotal,     // consistent with product sum
      collectableAmount: isCod ? boxInvoiceTotal : 0,
      boxCount: 1,
      products,
    },
  ]
  ...
  const result = await addSingleOrder({
    ...
    orderDetail: {
      ...
      shipmentInvoiceAmount: boxInvoiceTotal,        // = sum(box invoices) ✓
      totalCollectableAmount: isCod ? boxInvoiceTotal : 0,  // <= shipmentInvoiceAmount ✓
      boxDetails,
    },
  })
