...
const LISTING_WITH_CATEGORY = `${LISTING_PRODUCT_SELECT}, ${PRODUCT_CATEGORY_EMBED_FULL}`
...
async function fetchHomeCatalog() {
  ...
  // Only the rails actually rendered on the homepage (Shop All / latest rails are unused)
  ...
  const imageMap = await fetchListingImages(
    supabase,
    [...bestsellers, ...offeredProducts].map((p) => p.id)
  )
  ...
  return {
    ...
    // Kept for backwards compatibility with commented ShopAllThingsHome
    products: [],
    allProducts: [],
  }
}

export const getCachedHomeCatalog = unstable_cache(fetchHomeCatalog, ['home-catalog-v3'], {
  ...CACHE_OPTS,
