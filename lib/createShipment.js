...
export async function createShipmentForOrder(orderId, supabase, { paymentMethod = 'Prepaid' } = {}) {
  ...
  let addResult = await createBigShipOrder({
    ...
    // For COD: BigShip collects the full order total (subtotal + GST + delivery) from customer.
    // For Prepaid: invoice amount is the product subtotal only.
    totalAmount: order.payment_method === 'cod' ? (order.total || order.subtotal) : (order.subtotal || order.total),
