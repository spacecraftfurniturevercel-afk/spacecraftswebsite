...
// All admin recipients (deduplicated)
...
export async function sendMissingShippingDimsAlert({ products, orderId }) {
  const rows = products.map(p => `
    <tr>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:14px;">${p.id}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:14px;">${p.name}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:14px;color:#dc2626;">${p.missing.join(', ')}</td>
    </tr>`).join('')

  const html = emailWrapper('⚠️ Missing Shipping Dimensions', `
    <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:16px;margin-bottom:20px;">
      <strong style="color:#856404;">⚠️ Shipping Dimensions Missing — Order #${orderId}</strong>
      <p style="margin:8px 0 0;color:#856404;font-size:14px;">This order was created using default fallback dimensions (5kg, 30×20×20cm) because the following products are missing shipping data. Actual courier charges may differ.</p>
    </div>
    <p style="color:#374151;font-size:14px;margin-bottom:12px;">Please update these products in the admin panel with correct weight (kg) and box dimensions (cm):</p>
    <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:14px;">
      <tr style="background:#f9fafb;">
        <th style="padding:8px 12px;text-align:left;font-size:12px;color:#6b7280;font-weight:600;">Product ID</th>
        <th style="padding:8px 12px;text-align:left;font-size:12px;color:#6b7280;font-weight:600;">Product Name</th>
        <th style="padding:8px 12px;text-align:left;font-size:12px;color:#6b7280;font-weight:600;">Missing Fields</th>
      </tr>
      ${rows}
    </table>
    <p style="margin:20px 0 8px;font-size:13px;color:#6b7280;">Fields needed: <strong>shipping_weight</strong> (kg), <strong>shipping_length</strong>, <strong>shipping_width</strong>, <strong>shipping_height</strong> (cm).</p>
    <a href="${SITE_URL}/admin/products" style="display:inline-block;margin-top:8px;padding:10px 20px;background:#1a1a1a;color:#fff;border-radius:6px;text-decoration:none;font-size:14px;">Go to Admin Products →</a>
  `)

  return sendEmail({
    to: ADMIN_EMAILS,
    subject: `⚠️ Missing Shipping Dimensions — Order #${orderId} | ${SITE_NAME}`,
    html,
  })
}
