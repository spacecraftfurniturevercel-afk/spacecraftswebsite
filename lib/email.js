...
async function sendEmail({ to, subject, html, text }) {
  if (!SENDGRID_API_KEY) {
    console.log('SENDGRID_API_KEY not configured; skipping email to:', to)
  ...
  const body = {
    ...
    from: { email: EMAIL_FROM, name: SITE_NAME },
    content,
  }
  ...
  if (!res.ok) {
    ...
    return { ok: false, reason: 'sendgrid_error', status: res.status, text: errText }
  }
  return { ok: true }
}
